...
async function Load_File(f) {
  ...
  let file = f.files[0], ru = new FileReader(), ra = new FileReader();
...
async function Load_pdfDoc() {
  ...
  // qs("#pages-input").classList.add("bg-primary-subtle");
  pdfDoc = await PDF.load(oldPdf);
  pdfLength = pdfDoc.getPages().length;
}

// ACTIONS
function Pages(action, deg = 0) {
  let pgInput = qs("#pages-input").value, splitArr = Split_Text(pgInput), delArr = [];
  let inpStr = pgInput.replaceAll(' ', '').replaceAll(',', '').replaceAll(';', '').replaceAll('-', '');
  // if (isNaN(inpStr)) alert(`Error In Page Range \n Range Format: 1, 3-5 ; 8 \n\n`);
  if (isNaN(inpStr)) Msgbox("שגיאה בהזנת טווח","פורמט טווח: 1, 3-5 ; 8");
  // else if(pdfName==undefined) alert("Load PDF File");
    ...
    if (action == "extract")
      ...
      let allPg = [...Array(pdfLength).keys()].map(i => i);
...
function SaveAs() {
  if (saveFile.length>0) {
    let a = qs("#save-as-link"); a.href = saveFile; a.download = pdfName;}
  else
